...
import { Cpu, Lock, Eye, EyeOff, ArrowLeft } from 'lucide-react';
...
export default function ResetPassword() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      // 1. Check if hash params indicate recovery link
      ...
      if ((hasAccessToken && isRecovery) || isReset) {
      ...
      if (session) {
        ...
        toast.error('Access denied. No active password recovery session was found.');
        navigate('/login');
      }
      setChecking(false);
    };
    ...
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-[#030303] text-white p-4 relative overflow-hidden bg-grid-pattern">
        <div className="flex flex-col items-center justify-center space-y-4">
          ...
          <p className="text-sm text-[#9ca3af]">Verifying recovery session...</p>
        </div>
      </div>
    );
  }
      ...
      >
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-[#9ca3af]">
            Enter your secure new password credentials
          </p>
        </div>
        ...
        <div className="glass-panel rounded-2xl border border-[#1f1f23] p-8 shadow-2xl relative">
          {isValidSession ? (
            ...
            <div className="text-center py-4 space-y-4">
              <p className="text-sm text-red-400">Invalid or expired session. Please request a new password reset link.</p>
